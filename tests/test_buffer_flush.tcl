...
# --- the clock starts with the first event ----------------------------------
reset 1000 10 1000
enq {{"a":1}}
assert {[llength $::POSTS] == 0} "the first event after a restart is held for an interval"
advance 10
enq {{"a":2}}
assert {[llength $::POSTS] == 1} "and leaves once it has waited one"
assert {[post_body 0] eq {{"events":[{"a":1},{"a":2}]}}} "with what arrived meanwhile"
...
# A failed POST backs off as a missing token does. Without it the next due
# caller retries at once - on the traffic path, a request paying api_timeout.
reset 1 10 1000
set ::POST_RESULT "500"
enq {{"a":1}}
assert {[tbl "backoff"] ne ""} "a failed POST starts a backoff"
set ::POST_RESULT "200"
enq {{"a":2}}
assert {[llength $::POSTS] == 1} "nothing is attempted while backed off after a failure"
advance 31
enq {{"a":3}}
assert {[llength $::POSTS] == 2} "flushing resumes once the failure backoff expires"
assert {[post_body 1] eq {{"events":[{"a":2},{"a":3}]}}} "events held through the backoff go together"

# While backed off nothing can be sent, so nothing is due. Otherwise every
# request of an outage would walk into the flush to be turned away at its gate.
reset 1 10 1000
set ::TOKEN ""
enq {{"a":1}}
advance 10
assert {[_BUFFER_EVENT $::SUB {{"a":2}} 1 10 1000] == 0} "a deferring caller is not due while backed off"
advance 31
assert {[_BUFFER_EVENT $::SUB {{"a":3}} 1 10 1000] == 1} "and is due again once the backoff lifts"
...
# The token path keeps the buffer, so it stays due. The backoff is what keeps
# every further event from costing a token request.
set ::LOGS [list]
enq {{"a":2}}
enq {{"a":3}}
...
# --- the aged tail is the other backstop ------------------------------------
# The close path gets the first interval; a request flushes inline at two.
reset 1000 10 1000
enq_defer {{"a":1}}
assert {[llength $::POSTS] == 0} "inside the interval the caller still defers"
advance 10
enq_defer {{"a":2}}
assert {[llength $::POSTS] == 0} "the first interval belongs to the close path"
advance 10
enq_defer {{"a":3}}
assert {[llength $::POSTS] == 1} "at twice the interval the caller flushes inline, with no close involved"
assert {[post_body 0] eq {{"events":[{"a":1},{"a":2},{"a":3}]}}} "and takes the waiting events with it"

# A trickle on a connection nothing closes: every event has to leave.
reset 1000 10 1000
...
assert {[llength $::POSTS] == 3} "one POST per gap past the backstop, and one for the close"

# A caller whose connections do close never flushes from the request path.
reset 1000 10 1000
set from_request 0
for { set i 1 } { $i <= 5 } { incr i } {
...
# Cold start: the first event is held, and the backstop bounds the hold.
reset 1000 10 1000
enq_defer {{"a":1}}
assert {[llength $::POSTS] == 0} "the first event after a restart waits for a close"
advance 20
enq_defer {{"a":2}}
assert {[llength $::POSTS] == 1} "and is not held past the backstop when none comes"

# --- the tail of a burst ------------------------------------------------------
# Under the batch size, closing as it goes, then silence (#48: 1 of 20 left).
reset 1000 10 5000
for { set i 1 } { $i <= 20 } { incr i } {
    enq_defer [format {{"a":%d}} $i]
    closed
}
assert {[llength $::POSTS] == 0} "a burst under the batch size waits for the interval"
advance 10
closed
assert {[llength $::POSTS] == 1} "the first close after the interval drains it"
set body [post_body 0]
set got 0
for { set i 1 } { $i <= 20 } { incr i } {
    if { [string match [format {*{"a":%d}*} $i] $body] } { incr got }
}
assert {$got == 20} "every event of the burst is in that batch ($got of 20)"

# A flush resets the clock, or every close after one would POST.
enq_defer {{"a":21}}
closed
assert {[llength $::POSTS] == 1} "an event buffered after a flush is not due at the next close"
advance 10
closed
assert {[llength $::POSTS] == 2} "it is due one interval after it arrived"

# The clock is the oldest event's, so a trickle cannot keep pushing it away.
reset 1000 10 5000
for { set i 1 } { $i <= 5 } { incr i } {
    enq_defer [format {{"a":%d}} $i]
    closed
...
assert {[llength $::POSTS] == 1} "a trickle flushes once the oldest event has waited the interval"

# A keep-alive connection can hold its close for the TCP idle timeout (300 s).
reset 1000 10 5000
enq_defer {{"a":1}}
advance 300
closed
assert {[llength $::POSTS] == 1}                  "a close after an idle timeout still finds the event"
assert {[post_body 0] eq {{"events":[{"a":1}]}}} "rather than an expired slot"
...
# The remainder reads as overdue, so the next close takes the next slice.
closed
assert {[tbl "cursor"] == 6} "the next close resumes where the last flush stopped"
_FLUSH_EVENTS $::SUB
_FLUSH_EVENTS $::SUB
...
# --- events that expired before anything sent them ----------------------------
# An event held past its TTL leaves an empty slot. The flush skips it so the
# cursor cannot wedge, which used to mean the loss was invisible: "dropped"
# only ever counted the buffer-full path, so no warning read as no loss.
reset 1000 10 5000
enq [ev 1]
enq [ev 2]

# Past the TTL of 2T + 360, with no flush in between.
advance 400
enq [ev 3]
_FLUSH_EVENTS $::SUB

assert {[llength $::POSTS] == 1}                 "the surviving event is still sent"
assert {[string match {*-body {{"events":\[{"e":"xxx"}\]}}*} [lindex $::POSTS 0]]} \
    "only the event that had not expired is in the batch"
assert {[logged "*slot(s) empty at flush*"] == 1} "expiry is reported rather than silent"
assert {[logged "*buffer full*"] == 0}            "and it is not miscounted as a buffer-full drop"

# A flush that loses nothing must stay quiet, or the warning is noise.
reset 1000 10 5000
enq [ev 10]
_FLUSH_EVENTS $::SUB
assert {[llength $::POSTS] == 1}                  "a clean flush still sends"
assert {[logged "*slot(s) empty at flush*"] == 0} "and says nothing about empty slots"
