...
unset ms_traffic_reported
set ::ENQUEUED [list]
# Stub info hostname; everything else info does is passed on.
rename ::info ::info_real
proc ::info {sub args} {
    if { $sub eq "hostname" } { return "bigip-runtime.example.net" }
    return [uplevel 1 [linsert $args 0 ::info_real $sub]]
}
fire_request GET example.com /login
rename ::info ""
rename ::info_real ::info
assert {[string match {*"source":\{"id":"bigip-runtime.example.net"*} [lindex $::ENQUEUED 0]]} \
...
# --- CLIENT_ACCEPTED: the collect is the enable switch -------------------------
# Anything but 1 must not collect, an unsubstituted placeholder included.
set ::COLLECTS 0
namespace eval TCP { proc collect {args} { incr ::COLLECTS } }
proc accepted {} { set ::COLLECTS 0 ; uplevel #0 $::EVENT_BODY(CLIENT_ACCEPTED) }

set static::MIDEYE_SHIELD_traffic_enabled 1
set ms_traffic_ja3 "left over from the previous connection"
accepted
assert {$::COLLECTS == 1}              "enabled, CLIENT_ACCEPTED starts collecting"
assert {$ms_traffic_ch_pending == 1}   "and marks the collect as this iRule's"
assert {![info exists ms_traffic_ja3]} "and clears the previous connection's fingerprint"

foreach off {0 "__traffic_enabled__" ""} {
    set static::MIDEYE_SHIELD_traffic_enabled $off
    accepted
    assert {$::COLLECTS == 0}             "disabled ('$off'), CLIENT_ACCEPTED does not collect"
    assert {$ms_traffic_ch_pending == 0}  "and leaves nothing pending for CLIENT_DATA"
}
set static::MIDEYE_SHIELD_traffic_enabled 1

# --- HTTP_REQUEST with the collect still outstanding ---------------------------
# Another iRule released it first: warn once per connection, still report.
set ::WARNINGS [list]
proc LOG_WARNING {args} { lappend ::WARNINGS [join $args] }
set ms_traffic_ch_pending 1
unset -nocomplain ms_traffic_ja3 ms_traffic_ja4 ms_traffic_tls_version ms_traffic_alpn ms_traffic_reported
set ::ENQUEUED [list]
fire_request GET example.com /login
assert {[llength $::WARNINGS] == 1}                        "an outstanding collect at the first request is warned about"
assert {[string match "*no ClientHello captured*" [lindex $::WARNINGS 0]]} "naming the cause"
assert {$ms_traffic_ch_pending == 0}                        "and the collect is marked handled"
assert {[llength $::ENQUEUED] == 1}                         "the request is still reported"
assert {![string match {*"ja4":*} [lindex $::ENQUEUED 0]]}  "without a fingerprint it does not have"
fire_request GET example.com /next
assert {[llength $::WARNINGS] == 1}                         "a later request on the connection does not repeat it"
