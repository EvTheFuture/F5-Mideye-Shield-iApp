...
#   trailer  - bytes after the extension block, which no valid hello has
proc build_hello {ciphers extlist {rndchar A} {sessid ""} {legver 0x0303} {trailer ""}} {
    ...
    append body [be16 [string length $exts]] $exts $trailer
...
# The reference ClientHello behind FoxIO's published JA4 example: 1 GREASE + 15
# ciphers, 16 extensions, ALPN h2, TLS 1.3 via supported_versions. Its JA4 is an
# external value, which is what makes test_parser_sanity a check of the spec.
# Its JA3 has no published equivalent and is pinned to our own output.
