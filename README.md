...
| `MIDEYE_SHIELD_TRAFFIC` | an HTTP (or HTTPS) Virtual Server to fingerprint | Reports JA3/JA4 TLS client fingerprints and request metadata for **all** traffic, not just authentication attempts. On by default, wherever you attach it. |
...
It also needs an **HTTP profile**: events are built in `HTTP_REQUEST`, so a TLS
Virtual Server without one parses the handshake and then reports nothing.
...
make check-version      # the version rule below
```

**The version is one patch level above the upstream master.** Everything
here reaches the public repository as a single pull request, so the template
carries that pull request's version: whatever
`EvTheFuture/F5-Mideye-Shield-iApp` master has, plus 0.0.1, for every PR in
this repository until that one merges. CI checks it on every pull request.
...
The embedded copy of each iRule keeps its header and drops the other comments
and blank lines. tmsh refuses a rule body over 65535 characters, and
`MIDEYE_SHIELD_COMMON` is past that with its comments in; the build fails if a
stripped body comes within 2 kB of the limit. `tests/test_deployed_bodies.tcl`
checks that each stripped copy defines exactly what its source does.
...
**The deploy is tested too.** `tests/test_deploy_dry_run.tcl` runs the
template's implementation block against stubbed `tmsh` commands: every iRule
is created, each body is under tmsh's 65535-character limit after real
substitution, no placeholder survives, and answers with quotes or backslashes
reach their statics intact. It is the only thing that runs the tmsh side
before a BIG-IP does.
